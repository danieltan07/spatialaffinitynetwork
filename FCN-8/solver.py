...
class Solver(object):
    ...
    def __init__(self, data_loader, config):
        ...
        self.flowers_loader = data_loader["Flowers"]
    ...
    def train(self):
        ...
        if self.dataset == 'CelebA':
            ...
            self.data_loader = self.flowers_loader
        else:
        ...
        fixed_x = []
        real_c = []
        for i, (images, labels, identity) in enumerate(self.data_loader):
        ...
        if self.dataset == 'CelebA':
            ...
            fixed_c_list = self.make_flowers_labels(real_c)
        ...
        for e in range(start, self.num_epochs):
            for i, (real_x, real_label, identity) in enumerate(self.data_loader):
                ...
                real_gan, real_feature = self.D(real_x)

                d_loss_real = -torch.mean(real_gan)

                d_loss_feature = F.binary_cross_entropy_with_logits(
                    real_feature, real_label, size_average=False) / real_x.size(0)

                # Compute loss with fake images
                fake_x = self.G(real_x, fake_c)
                fake_x = Variable(fake_x.data)
                fake_gan, fake_feature = self.D(fake_x)

                d_loss_fake = torch.mean(fake_gan)
                d_loss_gan = d_loss_real + d_loss_fake

                # Backward + Optimize
                d_loss = d_loss_gan + d_loss_feature
                self.reset_grad()
                d_loss.backward()
                ...
                loss['D/loss_feature'] = d_loss_feature.data[0]
                ...
                if (i+1) % self.d_train_repeat == 0:
                    ...
                    # Compute losses
                    fake_gan, fake_features = self.D(fake_x)
                    g_loss_gan = - torch.mean(fake_gan)
                    g_loss_cycle = torch.mean(torch.abs(real_x - rec_x))


                    g_loss_feature = F.binary_cross_entropy_with_logits(
                        fake_features, fake_label, size_average=False) / fake_x.size(0)

                    # Backward + Optimize
                    g_loss = g_loss_gan + 10 * g_loss_cycle + g_loss_feature 
                    ...
                    # Logging
                    loss['G/loss_gan'] = g_loss_gan.data[0]
                    loss['G/loss_cycle'] = 10 * g_loss_cycle.data[0]
